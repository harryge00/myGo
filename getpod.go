...
import (
	...
	marathon "github.com/harryge00/go-marathon"
)
...
func init() {
	flag.StringVar(&marathonURL, "url", "http://master.mesos:8080", "the url for the marathon endpoint")
...
func main() {
	...
	pod, err := client.Pod("/productpage")
	if err != nil {
		fmt.Errorf("Pod err: %v", err)
	} else {
		fmt.Println("pod ", pod)
	}
	podStatus, err := client.PodStatus("/productpage")
	if err != nil {
		fmt.Errorf("podStatus err: %v", err)
	} else {
